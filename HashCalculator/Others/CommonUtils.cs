...
internal static class CommonUtils
{
    ...
    public static bool OpenFolderAndSelectItem(string path)
    {
        ...
        if (Directory.Exists(path))
        {
            ...
            shellExecuteInformation.fMask = SEMaskFlags.SEE_MASK_IDLIST |
                SEMaskFlags.SEE_MASK_FLAG_NO_UI | SEMaskFlags.SEE_MASK_ASYNCOK;
            shellExecuteInformation.hwnd = MainWindow.WndHandle;
            shellExecuteInformation.lpIDList = nativePath;
            shellExecuteInformation.nShow = ShowCmd.SW_SHOWNORMAL;
            bool boolResult = SHELL32.ShellExecuteExW(ref shellExecuteInformation);
    ...
    public static bool OpenFolderAndSelectItems(string folderPath, IEnumerable<string> files)
    {
        ...
        {
            return OpenFolderAndSelectItem(folderPath);
        }
        ...
        {
            intResult = SHELL32.SHOpenFolderAndSelectItems(folderID, (uint)fileIDList.Count,
    ...
    public static bool ClipboardSetText(Window owner, string text)
    {
        ...
        {
            NotificationSender.ShowMessageBox(owner, "复制失败", reasonForFailure);
        }
