...
/// <summary>
/// 用于 DataGrid 行的自定义单元格背景：values[0] 为 HashColorID，values[1] 为所在行 IsMouseOver。
/// 鼠标悬浮时返回透明（让行容器 DGR_Border 的悬浮背景色单层透出，避免与单元格叠加产生深浅割裂），
/// 非悬浮时返回 HashColorID 对应颜色（null 时为透明）。
/// </summary>
internal class HashColorHoverToBrushCvt : IMultiValueConverter
{
    public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
    {
        Debug.Assert(values != null && values.Length == 2);
        if (values[1] is bool isMouseOver && isMouseOver)
        {
            return new SolidColorBrush(Colors.Transparent);
        }
        if (values[0] is ComparableColor color && color.Color != default(Color))
        {
            return new SolidColorBrush(color.Color);
        }
        return new SolidColorBrush(Colors.Transparent);
