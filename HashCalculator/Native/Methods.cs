...
internal static class SHELL32
{
    ...
    /// 按 Windows 规则把一条命令行拆分为参数数组，与 CLR 传给 Main/e.Args 的拆分结果一致。
    /// 返回的指针数组需调用方用 KERNEL32.LocalFree 释放。
    /// </summary>
    [DllImport("shell32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    internal static extern IntPtr CommandLineToArgvW([MarshalAs(UnmanagedType.LPWStr)] string lpCmdLine,
        out int pNumArgs);
}
