...
1. 解决同时进行的任务数上限大于 1 时计算大批量文件会出现部分文件哈希值不正确的问题

## 版本：v5.29.1，2025/2/18

1. 解决同时进行的任务数上限大于 1 时计算大批量文件会出现部分文件哈希值不正确的问题

## 版本：v6.1.2，2025/2/6

1. 主窗口左下角的【加载中】动图改为圆形进度条，删除筛选窗口的【加载中】动图（解决小概率关闭主窗口后进程未退出导致可能无法再次启动的问题）

## 版本：v6.1.1，2025/1/26

#### 功能更改
1. 引入 HandyControl 及对部分控件及样式进行适配（界面更美观）
2. 内嵌依赖库改为由 Costura.Fody 实现
3. 更改部分提示对话框为 HandyControl 风格的对话框或 Growl 提示
4. 当成功复制哈希值或结果时在主窗口右上角弹出悬浮提示
5. 右键菜单扩展模块不兼容提示由窗口提示改为 Growl 消息提示
6. 给设置面板的分页标题增加图标
7. 改进右键菜单扩展模块兼容性提示逻辑
8. 调整默认算法面板布局
9. 去除主窗口标题中除软件名称外的其他信息
10. 算法库 hashalgs.dll 的释放和加载改为由 Costura.Fody 完成
11. 更新算法库文件 hashalgs.dll
12. 哈希值详情窗口中过长的哈希值改为不分行显示
13. 【相同文件夹筛选器给文件名列上色】由更改背景色改为在文件名图标前加竖向色条
14. 【相同哈希值筛选器给哈希值列上色】由更改背景色改为在哈希值前加竖向色条
15. 更改【重置所有颜色亮度和饱和度值】所更改的默认值大小
16. 设置项【显示新添加到表格的文件的图标】重命名为【显示表格中文件的图标】，并改为设置即时生效
17. 【用颜色来标记哈希值相同的行时的颜色参数】改为使用行前竖线标记，使标记在选中色干扰中显得更清晰
18. 检验结果由显示色块改为显示图标
19. 校验报告的显示由“光标悬浮于校验结果列中任意校验结果上显示”改为“光标悬浮于校验结果列表头上显示”

#### 新增功能
1. 添加设置项【通过右键/命令启动计算时使用已经复制的校验信息进行校验】并实现功能
2. 【常规设置】增加设置项【拖动文件到软件可执行文件上/启动命令没有子命令时】并实现功能

## 版本：v5.29.0，2025/1/26

#### 功能更改
1. 内嵌依赖库改为由 Costura.Fody 实现（软件体积变大的主要原因）
2. 改进右键菜单扩展模块兼容性提示逻辑
3. 去除主窗口标题中除软件名称外的其他信息
4. 算法库 hashalgs.dll 的释放和加载改为由 Costura.Fody 完成
5. 更新算法库文件 hashalgs.dll
...
4. 增加两个校验信息解析方案（通用五）和（通用六）
    - 想要看到新增的解析方案，需要重置解析方案。如果有自定义的解析方案，请先把它们复制到其他位置保存，重置方案后再重新添加，否则自定义方案将丢失！
5. 当无法从校验信息文件获知算法类型时，将默认依据的算法与启动校验时用户指定的算法相同
    - 当“校验信息文件内记录的哈希值长度”与“启动校验时指定的算法的结果应有的长度”不匹配时，或者启动校验时未指定算法，则行为仍然与旧版相同
6. 修复不清空表格的情况下选择校验信息手动进行校验时，如果校验信息内是文件相对路径而不是文件名，会导致校验结果是“无关联”的问题
...
2. 子命令 2：`verify`，用于从校验信息文件（文件哈希值清单）获知一批待计算哈希值的文件（文件名或相对路径）并计算，然后与校验信息文件内记录的哈希值进行对比，检查计算结果和校验信息文件内记录的哈希值是否一致。
...
6. 新增功能：当校验信息文件内容没有指定算法时，如果校验信息文件名以算法名作为扩展名，则自动选择文件扩展名指定的算法
7. 修正在上一次添加文件夹但未搜索完成的情况下再次添加另一个文件夹，需要多次点击取消搜索按钮才能停止搜索的问题
8. 新增支持将 .hash、.sfv、.sums 文件的“打开方式”设置为“哈希值校验信息”的功能（需要重新安装右键菜单扩展）
...
1. 修复作为校验信息打开文件后，找不到的文件在某些特殊情况下仍然会显示文件大小的问题
...
1. 更改部分筛选器的行为和名称
    ...
    - 【哈希值】筛选器更名为【搜索哈希值】
2. 更新预置的校验信息解析方案【通用一】的表达式(需要重置解析方案列表才能看见更新)
    - 现在哈希校验信息输入框的单个哈希值如果首尾包含空格也能正确解析
3. 修改 .sums 和 .hash 文件的预置解析方案，支持匹配带空格和小括号的算法名（需要重置解析方案才能看见更新）
4. 增强监控剪贴板并在合适添加下触发自动校验的功能
    - 该功能以前只能在复制单个哈希值时触发自动校验，现在只要是能被【校验信息解析方案】解析的内容，复制后都能触发自动校验，包括复制校验信息文件的完整路径
5. 更新文件相对路径的生逻辑，相对起始点与上一个本版有差异，影响所有使用 relpath 占位符的功能
    ...
    + 现在的版本：计算所有加入计算的对象的相对路径，相对起始点是加入计算的文件或文件夹的父目录，其中：
        ...
        - 将文本文件作为校验信息打开并进行自动校验时，视为将校验信息文件所在目录的多个文件/文件夹加入计算，所以相对起始点是这些文件/文件夹的父目录，即校验信息文件所在目录
6. 新增支持将带相对路径的哈希值清单文件作为校验信息文件
7. 导出哈希值结果的默认方案的一些更改
...
11. 新增功能：【校验信息解析方案】的【用于类型】支持多个扩展名
...
3. 增加按格式解析校验信息功能
    - 增加的功能的设置项位于设置面板的【校验信息解析方案】页中，可自定义如何解析输入的校验信息字符串或校验信息文件的内容
4. 修复文件不存在时无法右键复制文件名的问题，文件不存在时如果右键复制完整路径则弹出提示
5. 导出结果模板中的保存编码 “系统默认” 改为 ANSI（只是显示名称变化，编码并没有变化）
6. 扩展校验信息文件编码的支持范围，现在支持 UTF8/UTF16/ANSI(系统默认编码)/GB18030 及其兼容编码的文本文件
...
6. 修复把只包含哈希值的文件当作校验信息打开时获取到空文件名导致异常退出的问题
...
4. 解决误使用体积很大的非文本文件作为校验信息文件时长时间无法完成文件读取的问题
...
3. 给设置项“直接使用校验信息进行校验时首选依据算法而非默认算法”添加子项“当校验信息中未指明算法时”并实现相关功能
4. 用命令行参数启动校验流程的命令【hashcalculator.exe verify -b path_to_checklist_file】现在增加对可选参数【-a/--algo】的支持，此参数用于指定首选算法
5. 给系统右键菜单“作为哈希校验信息打开”添加算法子菜单，可以选择使用指定的算法来计算，其中“自动选择”子菜单行为与原行为相同
6. 修复当哈希值校验信息文件路径中有空格时从系统右键菜单启动校验没有反应的问题
...
5. 增加设置项“剪贴板中有新复制的哈希值时填充校验信息文本框并校验”
6. 设置面板增加设置项“用剪贴板哈希填充校验信息文本框并校验后把主窗带到前台”并实现功能
7. 主窗口的“校验信息”输入框支持粘贴/输入多行内容
8. 哈希校验信息输入框支持输入与导出的哈希结果文件一样的内容
9. 校验信息和命令行参数中的哈希算法名不再区分大小写
10. 修复某些情况下通过系统右键菜单计算文件哈希值或双击打开.hcb文件无法将已存在的HashCalculator主窗口带到前台的问题
11. 修复主窗口“哈希值校验信息”输入框混合输入单独的哈希值、哈希值+文件名、算法名+哈希值+文件名有可能会导致校验结果全部是“不确定”的问题
...
2. 实现通过哈希校验信息文件进行校验时同时校验多个算法的哈希值
3. 设置面板增加设置项“直接使用校验信息进行校验时首选校验信息中的算法而非默认算法”并实现从校验信息文件解析对应文件的对应算法并进行计算
4. 调整设置面板分类
...
9. 修复使用文本文件校验信息直接校验时，没有搜索到的文件名在表格中总是显示为小写字母的问题
...
2. 使用无效的校验信息文件时不再提示用户选择处理方式，而是中断校验并提示可能的失败原因
3. 其他 UI 细节调整


## 版本：v4.5.0，2023/7/23

1. 导出哈希结果的保存文件窗口增加[HashCalculator 校验信息|*.hcb]文件类型
...
6. 系统右键菜单增加选项"作为哈希校验信息打开"，对单个文件右击时右键菜单会出现此选项 (#5)
...
3. 校验信息文件读取逻辑更新：同时支持旧版和新版导出格式的文件
...
20. 删除“校验信息类型”下拉列表，"哈希值校验信息"输入框增加 PlaceHolder 类型的提示字符
...
8. 修复使用快速校验时，校验信息里存在但本地不存在的文件没有显示到表格里的问题。
...
3. 增加记住主窗口关闭时的位置设置项。
4. 增加同时进行的任务数量上限设置项。
5. 新增功能：如果面板为空且校验信息是通用格式的哈希值文件(注1)，可以直接点击校验而无需事先把文件拖入面板计算哈希值。点击校验后程序会在哈希值文件所在目录寻找相应文件计算哈希值并显示校验结果。
